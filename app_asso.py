...
app = FastAPI(title="API Gestion Tinka ka Mein Haaldi fotti", version="58.0")
...
def telecharger_recu_pdf(cotisation_id: int):
    ...
    c = res.data[0]
    # Sécurité supplémentaire : bloquer le téléchargement si non validé
...
def afficher_dashboard(id: Optional[int] = Query(None), filtre_periode: Optional[str] = Query(None)):
    ...
    try:
        ...
        for pr in all_projets:
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 mb-4 shadow-sm hover:shadow-md transition">
                <div class="flex justify-between items-start gap-2 mb-2">
                    <h4 class="font-black text-base text-indigo-950">{pr.get('titre', '')}</h4>
                    <span class="text-[11px] font-extrabold px-2.5 py-1 rounded-full bg-indigo-50 text-indigo-700 uppercase tracking-wide">{pr.get('statut', '')}</span>
                </div>
        ...
        if is_tresorier:
            ...
            for a in all_adherents:
                ...
                <tr class="hover:bg-slate-50 border-b text-sm">
                    ...
                    <td class="p-3 text-right">{actions_admin}<button onclick="openModal({a['id']})" class="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-1 rounded-xl text-xs font-bold ml-1 transition">⚙️</button></td>
                </tr>
                """
            ...
            # MENU DE NAVIGATION FIGÉ EN HAUT (Sticky Header)
            finance_sections_html = f"""
            <div class="sticky top-0 z-40 bg-white/95 backdrop-blur-md py-3 px-4 rounded-2xl shadow-md border border-slate-200/80 mb-6 flex flex-wrap gap-2">
            ...
            <div id="tab-tresorerie" class="tab-content space-y-6">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    <h2 class="text-base font-black mb-4 border-b pb-3 text-slate-800">💼 Trésorerie & Solde</h2>
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    <h2 class="text-base font-black mb-3 border-b pb-3 text-slate-800">📱 Paiements Mobile Money à valider</h2>
                    <div class="overflow-x-auto max-h-60 overflow-y-auto border border-slate-200 rounded-2xl">
                        <table class="w-full text-left bg-white"><thead class="bg-slate-100 text-[11px] font-bold text-slate-600 uppercase"><tr><th class="p-3">Adhérent</th><th class="p-3">Détails</th><th class="p-3">Montant</th><th class="p-3">Période</th><th class="p-3 text-right">Action</th></tr></thead><tbody>{paiements_mobiles_rows or '<tr><td colspan="5" class="p-4 text-center text-sm text-slate-400">Aucun paiement en attente.</td></tr>'}</tbody></table>
                    </div>
                </div>
            </div>
            ...
            <div id="tab-pointage" class="tab-content hidden space-y-6">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    ...
                    <form action="/presences-form" method="POST" class="space-y-4">
                        <input type="hidden" name="user_id" value="{user['id']}">
                        <div class="grid grid-cols-2 gap-3">
                            <div><label class="block text-xs font-bold mb-1 text-slate-600">Événement</label><select name="evenement_titre" required class="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm shadow-sm">{options_evenements_titres}</select></div>
                            <div><label class="block text-xs font-bold mb-1 text-slate-600">Date</label><input type="date" name="date_reunion" required class="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm shadow-sm"></div>
                        </div>
                        <div><label class="block text-xs font-bold mb-1 text-slate-600">Membre</label><select name="adherent_id" required class="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm shadow-sm" size="4">{options_presence_adherents}</select></div>
                        <div><label class="block text-xs font-bold mb-1 text-slate-600">Statut de présence</label><select name="statut_presence" required class="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm shadow-sm"><option value="Present">🟢 Présent(e)</option><option value="Absent_excuse">🟡 Excusé(e)</option></select></div>
            ...
            <div id="tab-projets" class="tab-content hidden space-y-6">
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    ...
                    <ul class="max-h-60 overflow-y-auto pr-2">{aides_admin_html or '<p class="text-sm text-slate-400">Aucune demande en attente.</p>'}</ul>
                </div>
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80">
                    <h2 class="text-base font-black mb-4 border-b pb-3 text-slate-800">🚀 Projets de l'Association</h2>
                    <form action="/projets-form" method="POST" class="space-y-3.5 mb-6">
                        <input type="hidden" name="user_id" value="{user['id']}">
                        <div class="grid grid-cols-2 gap-3">
                            <div><label class="block text-xs font-bold mb-1 text-slate-600">Titre</label><input type="text" name="titre" required class="w-full p-2.5 text-sm border border-slate-300 rounded-xl"></div>
                            <div><label class="block text-xs font-bold mb-1 text-slate-600">Coût (CFA)</label><input type="number" name="cout" required class="w-full p-2.5 text-sm border border-slate-300 rounded-xl"></div>
                        </div>
                        ...
                        <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl text-sm shadow-md transition">Ajouter un projet</button>
                    </form>
        ...
        if not is_tresorier:
            ...
            # CONDITION DEMANDÉE : Le reçu PDF n'est affiché/téléchargeable que si statut_paiement == 'valide'
            mois_payes_html = ""
            ...
            <div class="bg-gradient-to-br from-slate-900 via-emerald-950 to-slate-900 text-white p-6 rounded-3xl shadow-xl mb-6 flex justify-between items-center relative overflow-hidden">
                <div>
                    <span class="bg-emerald-500/20 text-emerald-300 text-[10px] font-extrabold px-2.5 py-1 rounded-full uppercase tracking-widest border border-emerald-500/30">Espace Membre</span>
            ...
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/85 mb-6">
